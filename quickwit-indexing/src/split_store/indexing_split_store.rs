...
impl IndexingSplitStore {
    ...
    pub fn without_local_store() -> Self {
        Self {
            remote_storages: Default::default(),
    ...
    pub async fn register_remote_storage(self, index_id: &str, storage: Arc<dyn Storage>) -> Self {
        self.remote_storages
            .lock()
            .await
            .insert(index_id.to_string(), storage);
        self
    }

    /// Returns the remote storage registered for `index_id`.
    async fn get_remote_storage(&self, index_id: &str) -> Arc<dyn Storage> {
        self.remote_storages
            .lock()
            .await
            .get(index_id)
...
mod test_split_store {
    ...
    use quickwit_storage::{
        PutPayload, RamStorage, SplitPayloadBuilder, Storage, StorageError, StorageErrorKind, OwnedBytes, RamStorageBuilder,
    };
    ...
    async fn test_multiple_storage_type() -> anyhow::Result<()> {
        let ram_storage_0 = RamStorage::builder()
        .put("foo", &[])
        .put("bar", &[])
        .build();
        let ram_storage_1 = RamStorage::builder()
        .put("foo", &[])
        .put("bar", &[])
        .build();
        let split_store = IndexingSplitStore::without_local_store()
            .register_remote_storage("test-index-0", ram_storage_0)
            .register_remote_storage("test-index-1", ram_storage_1)
            .await;
        split_store.delete("test-index-0", &Path::new("foo")).await;
        split_store.delete("test-index-1", &Path::new("bar")).await;
        assert!(!ram_storage_0.exists(&Path::new("foo")));
        assert!(ram_storage_0.exists(&Path::new("bar")));
        assert!(ram_storage_1.exists(&Path::new("foo")));
        assert!(!ram_storage_1.exists(&Path::new("bar")));
    }
}
